...
const MENU_ITEMS = [
  { id: 'dashboard', label: 'Painel Geral (Dashboard)', icon: Icons.dashboard },
  { id: 'pedidos',   label: 'Pedidos Online', icon: Icons.pedidos },
  { id: 'vendas',    label: 'Vendas Manuais', icon: Icons.vendas },
  { id: 'compras',   label: 'Registrar Compras', icon: Icons.compras },
  { id: 'estoque',   label: 'Gestão de Estoque', icon: Icons.estoque },
  { id: 'precos',    label: 'Preços de Catálogo', icon: Icons.precos },
              ...
              {showSearchBox && filteredNav.length > 0 && (
                <div style={{ position: 'absolute', top: 55, left: 0, right: 0, background: '#18181b', border: '1px solid #27272a', borderRadius: 12, zIndex: 100, boxShadow: '0 10px 30px rgba(0,0,0,0.5)', overflow: 'hidden' }}>
                   {filteredNav.map(item => (
                      <div 
                        key={item.id} 
                        onClick={() => { setTela(item.id); setSearch(''); setShowSearchBox(false); }}
